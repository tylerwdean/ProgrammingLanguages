defmodule Functional do
  def countWordsAndSentences(text) do
    ...
    IO.inspect(longestString(sentencesList))

    #Analyze the word counts,
    wordList = splitWords(text)
    totalWordCount = length(wordList)
    IO.puts("There are " <> Integer.to_string(totalWordCount) <> " words in this text sample. The longest one is: ")

    #Find the longest word
    IO.inspect(longestString(wordList))

    wordFreqMap = mapWords(text)
    uniqueWordCount = length(Map.keys(wordFreqMap))
    IO.puts("There are " <> Integer.to_string(uniqueWordCount) <> " unique words used. The most common words and their respective counts are: ")
    topTen = topTen(wordFreqMap)
    IO.inspect(topTen)
    IO.inspect(listOfValuesFromKeys(topTen, wordFreqMap))
  ...
  def splitIntoSentences(text, sentences \\ [], currentSentence \\ <<>>) do
    ...
    if remChars == 0 do
      ...
      {newSentences, word} = case currentChar do
        c when c in ~c".!?" ->
          ...
          trimmedSentence = String.trim(currentSentence, " ")
          newSentences = sentences ++ [trimmedSentence]
          {newSentences, <<>>}
        _ ->
          {sentences, currentSentence <> <<currentChar::8>> }
      end
  ...
  def longestString(list, i \\ 0, longestIndex \\ 0) do
    if (i >= length(list)) do
      Enum.at(list, longestIndex)
    else
      if (byte_size(Enum.at(list, i)) > byte_size(Enum.at(list, longestIndex))) do
        longestString(list, i+1, i)
      else
        longestString(list, i+1, longestIndex)
      end
    end
  end

  def listOfValuesFromKeys(keys, dict, valueList \\ [], i \\ 0) do
    if (i < length(keys)) do
      value = Map.get(dict, Enum.at(keys, i))
      valueList = List.insert_at(valueList, length(valueList), Integer.to_string(value))
      listOfValuesFromKeys(keys, dict, valueList, i+1)
    else
      valueList
    end
  end
...
Functional.countWordsAndSentences(lorem)
